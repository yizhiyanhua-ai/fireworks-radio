...
这次真实踩到的一个问题是：切换源的时候，**只杀 PID 文件里记住的那个进程并不够**。

原因很简单：

- 前台试播的 `mpv` 不一定会被写进 PID 文件
- 后面切到新源时，如果只停“上一次记住的那条”，残留的前台 `mpv` 还会继续放
- 结果就是用户耳边听到两条音频叠在一起

所以现在这条经验已经变成默认规则：

- 切换源前，不只停保存下来的 PID
- 还会额外清掉残留的 `mpv --no-video`
- 目标不是“尽量不重叠”，而是**默认不允许重叠**
...
这不是只针对 AI 新闻，而是 `fireworks-radio` 的所有音频播放都默认遵守的规则。

硬规则是：

- 只用公开、当前可访问、已经存在、可直接播放的源
- 不自己把网页内容重新合成为一段本地播报，除非用户明确要求
- 不为了播放去打开网页页面
- 优先直接在当前窗口里用终端播放器播音频
- 一次先播一个已经成型的源，不拼装伪播客或伪节目
- 第一条源跑通后，要顺手确认至少一个备援源

允许的源类型：

- 公开 YouTube 视频，只播音频
- 带公开 RSS feed 的播客
- 播客宿主直接暴露的 MP3 `enclosure` 音频
- 已经公开发布、可直接访问的免费音乐源
...
- 打开浏览器页面让用户自己点播放
- 把多个网页拼成“假节目”
- 没有用户明确要求时，扩成文章精读、Newsletter 摘要或别的非播放形式

### 音乐源补充

如果是音乐，这个 skill 可以优先找一些免费且公开的源类型，比如：
...
但要说清楚一点：目录是免费或公开，不等于任何单首内容都自动允许任意下游用途。逐条内容的授权和平台规则还是要看实际来源。

## AI 新闻模式

AI 新闻只是上面这条总规则下的一个子模式，会再额外收紧：

- 优先公开 YouTube AI 新闻视频，直接 `mpv --no-video`
- YouTube 不稳时，优先公开 podcast RSS / MP3 `enclosure`
- 如果这两类都失败，再解释原因，而不是偷偷切回本地合成

当前已经验证过的两类源：

- `YouTube`：适合 `mpv --no-video` 直接播公开 AI 新闻视频
- `Podcast RSS / MP3 enclosure`
  例如 `The AI News Daily Brief` 的公开 RSS：
  `https://feeds.podcastai.com/lzSp5oGSGEaLNzHdqUBaTh.xml`

这条 RSS 里已经能直接拿到 `enclosure` 音频直链，所以它可以作为 YouTube 之外的备援。
